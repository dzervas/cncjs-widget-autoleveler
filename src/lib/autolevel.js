...
module.exports = class Autolevel {
    ...
    start(cmd, context) {
        ...
        if (m) {
            this.delta = parseFloat(m[1]);
        }

        let h = /H([\.\+\-\d]+)/gi.exec(cmd);
        if (h) {
            this.height = parseFloat(h[1]);
        }

        let f = /F([\.\+\-\d]+)/gi.exec(cmd);
        if (f) {
            this.feed = parseFloat(f[1]);
        }
    ...
    updateContext(context) {
        if (this.wco.z !== 0 && context.mposz !== undefined && context.posz !== undefined) {
